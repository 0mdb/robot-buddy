...
occur.  Does NOT mutate face state directly — returns action descriptors
for the caller to dispatch.

Canonical implementation — supervisor port mirrors this file.
"""
...
from tools.face_sim_v3.state.constants import (
    ...
    TRANS_SL_NOD_DURATION_MS,
    TRANS_TS_BLINK_DELAY_MS,
    TRANS_TS_BLINK_DURATION_MS,
...
class TransitionAction:
    ...
    delay_ms: float  # ms from transition start
...
class ConvTransitionChoreographer:
    ...
    def on_transition(self, prev: ConvState, new: ConvState) -> None:
        """Load choreography for a state transition."""
        self._reset()

        if prev == ConvState.LISTENING and new == ConvState.THINKING:
            self._setup_listening_to_thinking()
        elif prev == ConvState.THINKING and new == ConvState.SPEAKING:
            self._setup_thinking_to_speaking()
        elif prev == ConvState.SPEAKING and new == ConvState.LISTENING:
            self._setup_speaking_to_listening()
        elif prev == ConvState.SPEAKING and new == ConvState.DONE:
    ...
    def get_gaze_override(self) -> tuple[float, float] | None:
        ...
        if self._gaze_ramp.done:
            return None
        return self._gaze_ramp.update(0.0)  # Read current value (dt=0)
    ...
    def _setup_listening_to_thinking(self) -> None:
        ...
        listening_gaze = CONV_GAZE[ConvState.LISTENING] or (0.0, 0.0)
        thinking_gaze = CONV_GAZE[ConvState.THINKING] or (0.5, -0.3)
        self._gaze_ramp = _GazeRamp(
            start_x=listening_gaze[0],
            start_y=listening_gaze[1],
            end_x=thinking_gaze[0],
            end_y=thinking_gaze[1],
    ...
    def _setup_thinking_to_speaking(self) -> None:
        ...
        self._actions = [
            TransitionAction(
                ...
                params={
                    "name": "blink",
                    "gesture_id": int(GestureId.BLINK),
        ...
        thinking_gaze = CONV_GAZE[ConvState.THINKING] or (0.5, -0.3)
        speaking_gaze = CONV_GAZE[ConvState.SPEAKING] or (0.0, 0.0)
        self._gaze_ramp = _GazeRamp(
            start_x=thinking_gaze[0],
            start_y=thinking_gaze[1],
            end_x=speaking_gaze[0],
            end_y=speaking_gaze[1],
    ...
    def _setup_speaking_to_done(self) -> None:
        ...
        self._actions = [
            TransitionAction(
                kind="mood_nudge",
                delay_ms=0.0,
                params={"mood_id": int(Mood.NEUTRAL), "intensity": 0.0},
            ),
        ]
