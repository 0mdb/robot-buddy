#pragma once
// BMI270 initialization config file — "maximum FIFO" variant (328 bytes).
// Strips step-counter, gesture, and activity features we don't use.
// Source: Bosch BMI270_SensorAPI v2.86.1 (BSD-3-Clause)
//   https://github.com/boschsensortec/BMI270_SensorAPI
//
// This blob MUST be burst-written to INIT_DATA (0x5E) on every POR or
// soft reset before the sensor will produce data.

#include <cstdint>

static const uint8_t BMI270_CONFIG_FILE[] = {
    ,
};

static constexpr size_t BMI270_CONFIG_FILE_SIZE = sizeof(BMI270_CONFIG_FILE);
